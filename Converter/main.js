...
const lengthButton = document.getElementById('length').addEventListener('click', (e) => {
	...
	toMetric.addEventListener('click', (e) => {
		output.innerHTML = `
		${input.value} foot &rarr; ${lengthUnits.imperial.foot()} m <br><br>
		${input.value} inch &rarr; ${lengthUnits.imperial.inch()} mm <br>
...
const massButton = document.getElementById('mass').addEventListener('click', (e) => {
	...
	toMetric.addEventListener('click', (e) => {
		output.innerHTML = `
		${input.value} pound &rarr; ${massUnits.imperial.pound()} kg <br><br>
		${input.value} ounce &rarr; ${massUnits.imperial.ounce()} gram <br>
...
const lengthUnits = {
	imperial: {
		...
		foot(){
			let footToM = (input.value * 0.3048).toFixed(2);
			return footToM;
		},
		yard(){
